...
%%
    % ====================================================
    % PRINT NUMERICAL DIAGNOSTICS (every 10 steps)
    % ====================================================
    if mod(round(t/dt), 10) == 0

        T0_check = double(Kuka.fkine(q_current));
        pos = T0_check(1:3,4);

        fprintf('\n===== t = %.3f s =====\n', t);

        % 1. END-EFFECTOR POSITION
        fprintf('Tip position     : X=%.2f  Y=%.2f  Z=%.2f mm\n', pos(1), pos(2), pos(3));

        % 2. ACTUAL TIP VELOCITY (what the robot is actually doing)
        V_actual = J_numerical * q_dot;
        fprintf('Desired Vz       : %.4f mm/s\n', V_desired(3));
        fprintf('Actual  Vz       : %.4f mm/s\n', V_actual(3));
        fprintf('Velocity error   : %.6f mm/s\n', abs(V_desired(3) - V_actual(3)));

        % 3. JACOBIAN CONDITION NUMBER (singularity warning)
        cond_J = cond(J_numerical);
        fprintf('Condition number : %.2f', cond_J);
        if cond_J > 1000
            fprintf('  <<< WARNING: near singularit y!');
        end
        fprintf('\n');

        % 4. JOINT ANGLES
        fprintf('Joint angles (deg): ');
        fprintf('%6.1f  ', rad2deg(q_current));
        fprintf('\n');

        % 5. JOINT VELOCITIES
        fprintf('Joint speeds (rad/s): ');
        fprintf('%7.4f  ', q_dot);
        fprintf('\n');
